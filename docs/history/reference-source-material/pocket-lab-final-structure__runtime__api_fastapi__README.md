# Pocket Lab FastAPI + NATS Runtime

The `runtime/api_fastapi` directory contains the active FastAPI control plane, NATS/JetStream integration, WebSocket event delivery, and worker-facing runtime documentation. The architecture is FastAPI-only for HTTP, NATS/JetStream-required for production writes, and typed-operation-only for browser mutation paths.
...
|---|---|
| Routers | REST, OpenAPI, readiness, health, release, fleet, catalog, drift, vault and operation routes. |
| Event bus | NATS/JetStream stream setup, publishing, subscription, audit, telemetry and DLQ handling. |
| Workers | Durable command consumption and operation/domain execution. |
| Workflow engine | Event-sourced journal, projections, command journal, replay/recovery. |
| WebSocket | Browser-facing live event delivery. |
