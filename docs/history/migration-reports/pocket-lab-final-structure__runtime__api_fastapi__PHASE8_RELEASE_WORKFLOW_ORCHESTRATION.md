# Phase 8 — Release Workflow Orchestration

Phase 8 turns release check/apply into a staged workflow executed through typed NATS commands and visible as release events.

## Flow

```text
POST /api/release/self-update/check|apply
  -> NATS release command
  -> worker/domain release orchestrator
  -> staged release events
  -> UI Release Workflow tab and LiveEventPanel
```

## Typical stages

Metadata fetch, prepare/backup, GitOps sync, catalog refresh, blueprint deploy, drift verify, health verify, final status, rollback evidence.
