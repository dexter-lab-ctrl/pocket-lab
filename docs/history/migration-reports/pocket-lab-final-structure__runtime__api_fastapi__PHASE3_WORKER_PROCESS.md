...
Phase 3 moves execution behind a worker process. FastAPI validates and queues work; the worker consumes durable commands and executes operation/domain handlers.

## Runtime flow

```text
React PWA
  -> FastAPI /api/operations/execute or typed domain route
  -> JetStream command subject
  -> Pocket Lab worker
  -> runtime/core service handler
  -> event publication + workflow journal
  -> WebSocket/UI progress
```

## Reliability expectations

- Worker execution is the default production path.
- If NATS/JetStream is unavailable, production writes fail closed.
- Worker status is visible through FastAPI status endpoints and live events.
