# Phase 2 — NATS Event Bus Wrapper

Phase 2 establishes NATS/JetStream as the event bus behind FastAPI while preserving browser-facing REST/OpenAPI and WebSocket boundaries.

## Responsibilities

- Connect to `POCKETLAB_NATS_URL`.
- Require NATS/JetStream for production command/event flows.
- Create streams for commands, events, audit, telemetry, and DLQ.
- Publish operation, health, telemetry, release, drift, fleet, and audit events.
- Keep the browser decoupled from direct NATS access.
