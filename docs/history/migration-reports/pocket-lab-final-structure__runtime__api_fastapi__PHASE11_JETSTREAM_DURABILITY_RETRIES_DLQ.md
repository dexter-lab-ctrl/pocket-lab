# Phase 11 — JetStream Durability, Retries and DLQ

Phase 11 makes command execution durable and observable.

## Runtime semantics

```text
FastAPI command submit
  -> JetStream stream POCKETLAB_COMMANDS
  -> durable consumer
  -> worker execution
  -> ack on success
  -> nak/retry on transient failure
  -> DLQ after max delivery attempts
```

## Requirements

- Explicit acknowledgement on successful processing.
- Bounded retry policy.
- Dead-letter events for operator visibility.
- Correlation IDs across command, events, logs and workflow journal.
