...
Phase 10 documents the fleet-agent model where devices join through controlled FastAPI/NATS workflows rather than ad hoc shell-driven enrollment.

## Flow

```text
Fleet UI / Simple Add Device
  -> FastAPI fleet route or typed operation
  -> pocketlab.commands.fleet.*
  -> worker/fleet handler
  -> agent state/events
  -> UI fleet status
```
...
Fleet subjects must be constrained by NATS permission policy. Device identity, enrollment keys, and trust material must be redacted from journals and logs.
