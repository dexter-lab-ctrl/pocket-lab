...
Phase 6 introduces explicit domain command subjects for catalog, drift, fleet, release, health, security, Vault, backup and policy actions.

## Contract

- Each domain command has an explicit subject.
- Each handler validates its payload.
- Each handler emits progress/result events.
- Failures move through retry/DLQ behavior instead of being hidden by local fallback.
