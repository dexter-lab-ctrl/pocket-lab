# Phase 9 — Event-native Health and Telemetry

Phase 9 makes System Status/NOC telemetry event-native instead of request-only.

## Behavior

- FastAPI samples health, telemetry, and fleet status.
- Events are published to NATS/JetStream.
- UI reads FastAPI snapshots and event streams.
- Gatus remains a health evidence source, not a direct browser dependency.

## Validation

The current baseline validates API contract, schema, WebSocket, frontend build, and visual rendering through `task test:visual`.
