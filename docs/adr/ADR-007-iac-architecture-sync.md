...
This report covers the IaC and GitOps structure that deploys and validates Pocket Lab's production-compatible control plane.

## Result

The IaC architecture is synchronized to the current runtime model:

- FastAPI is the only HTTP/API control plane.
- NATS/JetStream is a required production dependency.
- Caddy proxies REST and WebSocket control-plane traffic.
- Workers execute typed commands from durable streams.
- Gatus-backed health is consumed through FastAPI-facing health snapshots/events.
- Bootstrap and Ansible roles are idempotent and compatible with Android/Termux constraints where applicable.

## IaC responsibilities

| Area | Responsibility |
|---|---|
| `site.yml` | Orders platform roles and workload roles. |
| `inventory/*/group_vars/all.yml` | Defines environment-specific ports, URLs, health checks, and service configuration. |
| `roles/nats` | Provisions NATS/JetStream and stream dependencies. |
| `roles/fastapi_control_plane` | Deploys the FastAPI runtime and worker-facing configuration. |
| `roles/caddy` | Provides HTTP/WebSocket ingress routing. |
| `roles/vault` | Provides secrets bootstrap and secret-store integration. |
| `roles/catalog_seed` | Seeds the Apps & Services/blueprint catalog. |
| `roles/drift_check` | Supports desired-state drift validation. |
| `roles/backups` | Supports backup/restore state operations. |
