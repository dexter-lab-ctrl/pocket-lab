...
export default function CodeBlock({ code, id, filename = "bootstrap.sh" }) {
  ...
  return (
    <div className="relative group mt-5 animate-in fade-in slide-in-from-bottom-2 duration-500">
      ...
      <div className="flex items-center px-4 py-3 bg-[#0f1423] border border-white/10 border-b-0 rounded-t-2xl shadow-inner relative z-10">
      ...
      {/* Scrollable Code Area - Now using standard text rendering[cite: 17] */}
      <div className="relative bg-[#05080f] rounded-b-2xl border border-white/10 overflow-hidden shadow-[inset_0_0_30px_rgba(0,0,0,0.8)]">
      ...
      <button 
        ...
        className={`absolute top-14 right-4 p-2.5 rounded-xl border transition-all duration-300 shadow-xl flex items-center space-x-2 ${
          copiedId === id 
            ...
            : 'bg-white/10 backdrop-blur-md border-white/20 text-slate-200 opacity-0 group-hover:opacity-100 hover:bg-indigo-600 hover:border-indigo-400'
        }`}
      >
        {copiedId === id ? (
