...
export default function ControlPlaneBanner({ compact = false, simpleMode = false }) {
  ...
  const label = simpleMode ? (ready ? 'Pocket Lab is ready' : 'Pocket Lab needs attention') : (ready ? 'FastAPI/NATS control plane ready' : 'FastAPI/NATS control plane degraded');
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 text-sm leading-6 text-slate-300">{simpleMode ? (ready ? 'Installs, updates, backups, and device actions can run safely.' : 'Paused for safety. Actions are stopped so Pocket Lab does not make unsafe changes.') : status.message}</p>
            <ProgressiveDisclosure simpleMode={simpleMode} title={simpleMode ? 'Show readiness details' : 'Control-plane readiness details'} className="mt-3">
              ...
              <div className="flex flex-wrap gap-2">
                <StatusBadge status={status.api ? 'healthy' : 'degraded'} simpleMode={simpleMode}>FastAPI {status.api ? 'ready' : 'not ready'}</StatusBadge>
                <StatusBadge status={status.nats ? 'healthy' : 'degraded'} simpleMode={simpleMode}>NATS {status.nats ? 'connected' : 'offline'}</StatusBadge>
                <StatusBadge status={status.jetstream ? 'healthy' : 'degraded'} simpleMode={simpleMode}>JetStream {status.jetstream ? 'enabled' : 'required'}</StatusBadge>
                <StatusBadge status={status.worker ? 'healthy' : 'degraded'} simpleMode={simpleMode}>Worker {status.worker ? 'ready' : 'not ready'}</StatusBadge>
              </div>
