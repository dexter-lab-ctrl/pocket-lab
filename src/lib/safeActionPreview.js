const PREVIEWS = {
  deploy_blueprint: {
    risk: 'medium',
    will: ['Install the selected app or service from an approved blueprint.', 'Queue a typed operation through the FastAPI control API.', 'Record lifecycle events and audit evidence.'],
    ...
    evidence: ['Operation request', 'Worker lifecycle events', 'Audit record'],
  },
  git_sync: {
    risk: 'low',
    ...
    willNot: ['Expose a shell editor.', 'Talk directly to Git/NATS from the frontend.', 'Apply destructive changes without the worker path.'],
    evidence: ['Operation request', 'GitOps event', 'Audit record'],
  },
  drift_scan: {
    risk: 'low',
    will: ['Check for differences from desired state.', 'Show findings before changes are applied.', 'Use a preview-style typed operation.'],
  ...
  fleet_join: {
    risk: 'low',
    will: ['Prepare a safe device invite.', 'Queue device onboarding through FastAPI.', 'Record the join request and status.'],
  ...
  restore_backup: {
    risk: 'high',
    will: ['Prepare a restore request for the selected backup.', 'Require the worker-owned restore path.', 'Record restore evidence and status.'],
  ...
  release_sync: {
    risk: 'medium',
    will: ['Check and apply an approved release workflow.', 'Record release status and lifecycle events.', 'Use typed operation execution.'],
  ...
  rotate_secret: {
    ...
    willNot: ['Reveal secret values in the UI.', 'Store raw secrets in browser logs.', 'Bypass vault/worker ownership.'],
...
const DEFAULT_PREVIEW = {
  risk: 'medium',
  will: ['Queue this task through the FastAPI control API.', 'Let the worker own execution.', 'Record observable lifecycle events.'],
  willNot: ['Run shell commands from the frontend.', 'Talk directly to NATS from the browser.', 'Bypass audit evidence.'],
...
export function safeActionPreview(operation, { simpleMode = false } = {}) {
  ...
  return {
    ...preview,
    will: preview.will.map((item) => item.replace('FastAPI control API', 'Pocket Lab control plane').replace('typed operation', 'safe task')),
    willNot: preview.willNot.map((item) => item.replace('frontend', 'your browser').replace('NATS', 'backend messaging')),
