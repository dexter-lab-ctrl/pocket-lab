...
def validate(metadata: dict[str, Any]) -> list[str]:
    ...
    if not screens:
        errors.append("no screens defined in Tier 9 UI metadata")
...
def build_markdown(metadata: dict[str, Any], manifest: dict[str, Any]) -> str:
    ...
    lines.append("")
    lines.append("This Tier 9 reference is generated from Storybook UI documentation metadata and the `PocketLabTabs.stories.jsx` story file. It documents screen behavior with deterministic FastAPI mock responses, not manually captured stale screenshots.")
    lines.append("")
...
def build_primary_markdown() -> str:
    return """# Pocket Lab Screen-by-Screen UI/UX Manual\n\nThis page is generated from Tier 9 Storybook UI documentation metadata. Update `src/stories/tier9UiScreens.json` and `src/stories/PocketLabTabs.stories.jsx`, then run `task docs:ui`.\n\n--8<-- \"docs/product/generated/ui-screen-reference.generated.md\"\n"""
...
def inject_tier9a_screenshot_evidence(markdown: str) -> str:
    """Inject Tier 9A screenshot images into the generated UI manual.

    The screenshot manifest is generated by scripts/docs/capture_ui_storybook_screenshots.mjs.
    This function keeps Tier 9 usable without screenshots, but embeds them when evidence exists.
    """
...
def embed_tier9a_screenshot_evidence() -> None:
    """Embed Tier 9A screenshot evidence into the generated UI manual.

    This runs after the normal Tier 9 markdown generation so the UI manual remains
...
def main() -> None:
    ...
    print(f"Tier 9 UI documentation: screens={manifest['screen_count']} stories={manifest['story_export_count']}")
